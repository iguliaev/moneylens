...
**Date:** 2026-09-19
**Status:** In progress (code done, hosted rollout pending)
...
- **2026-09-19** — Steps 3–7 done in `e66cd8a8`; **Status: In progress — code complete, hosted rollout (steps 8–9) and roadmap tick (10) remain, all manual/after merge.** Findings: (a) the route-guard risk (§2.2) was **real** — after the link worked, `NavigateToResource` bounced the recovery session to the dashboard before the form showed; fixed by moving `/update-password` out of the guard in `App.tsx` (renders `EnvironmentBanner` + `AuthPage` itself). (b) Local `supabase/config.toml` now hard-codes `site_url = http://localhost:5173` and allow-lists `…/update-password` (comment warns against `supabase config push`). (c) Real email checked via Mailpit API (`http://127.0.0.1:54324/api/v1/…`, the local inbox is Mailpit now): button = `…/auth/v1/verify?token=…&type=recovery&redirect_to=http://localhost:5173/update-password`. (d) `.env.local` sets `BASE_URL`, which disables Playwright's auto-started dev server — `npm run dev` must be running for e2e locally. Verification: reset spec 2/2 pass; full suite 120 passed, 1 failed = `transactions.spec.ts` "category dropdown sorts by full hierarchy label", which is flaky and unrelated (5/6 pass on repeat; not touched). Lint 0 errors, `tsc` clean. Next: user does steps 8–9 by hand.
...
- [x] 4. **Templates** — `supabase/templates/recovery.html` and `magic-link.html`: replace the `href` with `{{ .ConfirmationURL }}`; keep the styling.
- [x] 5. **Guard fix if step 2 shows a bounce** (it did) — move the `/update-password` route out of the `Authenticated`/`NavigateToResource` group in `App.tsx`. Optionally gate the form on a `PASSWORD_RECOVERY` event from `onAuthStateChange`; skip unless step 2 shows a need (YAGNI).
- [x] 6. **Manual check via Inbucket/Mailpit** (done via API rather than a UI click-through; the link shape matches what `generateLink` exercises in the spec) (`http://localhost:54324`): request a reset from the UI, confirm the email's button URL is `…/auth/v1/verify?…&redirect_to=…/update-password`, click through, set a password.
- [x] 7. **Docs** — rewrite the stale parts of `docs/deployment/email-templates-setup.md` (variables `{{ .ConfirmationURL }}`, drop `token_hash`/`/auth/confirm`/`next` expectations), `docs/deployment/redirect-urls-setup.md` (add `…/update-password` entries for local, staging `git-main` and prod), `docs/deployment/password-reset-deployment-checklist.md` (drop "Monitor `/auth/confirm`"), and `docs/deployment/environment-variables.md` (local port 5173).
