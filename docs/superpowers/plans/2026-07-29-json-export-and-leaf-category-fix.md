...
- **2026-08-01** — Documentation-only follow-up: added an explicit "Known limitation" bullet to the Decisions section noting `insert_categories` doesn't support `parent_id`, so bulk/JSON import of a `"Parent/Child"` category path requires both categories to already exist — this was previously only stated in `docs/api/bulk-upload.md`, not in this plan. No code changes.
...
- **Known limitation, out of scope for this plan:** bulk/JSON import cannot *create* nested categories on the fly. `insert_categories` (the RPC backing `bulk_upload_data`'s `categories` section) only inserts root-level categories — it has no `parent_id` parameter. So a `"Parent/Child"` path in the `transactions` section only resolves if **both** the parent and the child category already exist beforehand (e.g. created via the UI, or via a prior/same-payload `categories` section for the parent, with the child created separately since there's no bulk way to create it with a parent). This is documented in `docs/api/bulk-upload.md` ("`insert_categories` does not support setting `parent_id`") but was not called out here or in the original hierarchy plan (`2026-06-01-parent-categories-closure-table.md`) — flagged here so it isn't lost. Scoping `insert_categories` to accept an optional `parent_id` would be natural follow-up work if bulk-creating nested category trees turns out to matter.
