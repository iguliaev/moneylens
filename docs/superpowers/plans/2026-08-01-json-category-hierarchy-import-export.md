...
- **2026-08-01** — Code review of PR #262 found three issues, all fixed in the same branch. (1) The Phase 2 parent lookup read `categories` without `deleted_at IS NULL`, so a new child could be nested under a soft-deleted parent — the exact class of bug documented in `docs/database/schema-and-migrations.md` after the `get_transaction_tags` incident (PR #257). Added the filter, and since the `unique_user_type_name` constraint counts soft-deleted rows (so Phase 1 can't auto-vivify a live replacement), that case now raises an explicit error instead. (2) `name` was never trimmed while `parent` was, and an unresolved parent was silently dropped by `WHERE pid IS NOT NULL` — names are now trimmed consistently and an unresolved parent raises rather than dropping the row, matching `bulk_insert_transactions`' loud-validation posture. (3) `collectJsonExportCategories` built its dedup key by unescaped `::` concatenation, so `{parent:"Food::Sub",name:"X"}` and `{parent:"Food",name:"Sub::X"}` collided and one was dropped from the export — now keyed by `JSON.stringify` of the tuple. Added pgTAP Tests 32–35 (soft-deleted parent rejected, child not inserted, whitespace trimming round-trip) and a Vitest collision case. Regression: 259/259 pgTAP, 39/39 Vitest, 109/109 Playwright e2e, lint and type-check clean.
