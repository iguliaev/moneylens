...
- [ ] Spend budgets always render red ("exception") under 80% utilization — `utility/budgetAlerts.ts` was refactored (2026-06-27) into `getBudgetAlertState` (warn ≥80%, over ≥100%) feeding `getProgressStatus`, so the 80–100% band now renders `"normal"` and ≥100% renders `"exception"`. The under-80% case is still wrong: `getProgressStatus` falls through to an explicit `if (type === "spend") return "exception"`, so a spend budget at any utilization below the warn threshold renders red as if it were over budget. Fix: return `"normal"` (neutral) for a spend budget below the warn threshold.
...
- [ ] Transaction-defaults Selects lose the write on a slow connection — surfaced 2026-08-31 while de-flaking `transaction-defaults.spec.ts` (#278). In `settings/index.tsx`, `DefaultValueSelect` is `value`-controlled via `valueIfStillAvailable(...)`, which returns `undefined` until the upsert is persisted *and* refetched; AntD then falls back to internal state, so a picked option paints optimistically with no in-flight/disabled state on the Select. The `setDefaultsForType` upsert (`hooks/useTransactionDefaults.ts`) has no `keepalive` and isn't retried, and an aborted request never reaches `notifyFailure` (it only fires on an error *response*). Net: a user who picks a default and immediately navigates away loses it silently. Fix options: keep the Select controlled with the just-picked value during the write, show a pending state, and/or make the upsert survive navigation and report failure. The PR #278 fix was test-only (`waitForDefaultsWrite`).
- [x] ~~Password reset (and magic-link) emails link to a 404~~ — reported 2026-09-19 on staging. A user enters a valid email on Forgot Password, gets the "Reset Your Password" email, but the button opens `https://<host>/auth/confirm?token_hash=…&type=recovery&next=/update-password` and Vercel returns `404 NOT_FOUND`, so the flow can't be completed. **Root cause (from code, not yet reproduced against staging):** `supabase/templates/recovery.html` (and `magic-link.html`) build links to `{{ .SiteURL }}/auth/confirm`, but nothing serves that path any more. It was a Next.js route handler (`apps/web/src/app/auth/confirm/route.ts`, which verified the `token_hash` server-side) deleted with the old web app in #120. The Vite SPA in `apps/web-next` has no `/auth/confirm` route in `App.tsx`, no `api/` serverless function, and `vercel.json` has no rewrites, so the request 404s at Vercel before the SPA loads. Likely affects magic-link login the same way (`next=/dashboard`); worth checking production too. **Chosen fix: option (b), Supabase's default `{{ .ConfirmationURL }}` flow** — this is the documented path for a client-side SPA (Supabase's password docs: `resetPasswordForEmail` + `redirectTo` → `PASSWORD_RECOVERY` event → `updateUser`; the `token_hash`/`/auth/confirm` template is the SSR/PKCE variant the old Next.js app used). Refine's stock Supabase `authProvider` works the same way, and ours already matches (`forgotPassword` passes `redirectTo: <origin>/update-password`, `updatePassword` calls `auth.updateUser`). Steps: (1) switch `supabase/templates/recovery.html` (and `magic-link.html`) `href` to `{{ .ConfirmationURL }}` and re-apply both templates in the hosted staging/prod Supabase dashboards (they don't read `supabase/templates/`); (2) add `<origin>/update-password` (and `<origin>/dashboard` if magic links keep a `next`) to the Redirect URLs allow-list per environment, e.g. a wildcard for Vercel preview URLs — `config.toml`'s `additional_redirect_urls` only has `SITE_URL`, and a non-allow-listed `redirect_to` silently falls back to the Site URL so the user would never see the password form; (3) **verify against a real recovery link** that the `/update-password` route isn't bounced by the `<Authenticated fallback={<Outlet/>}><NavigateToResource/>` guard in `App.tsx` (~L288) — the recovery session makes the user "authenticated", so it may redirect them off the form; if so, move the route out of that group (unverified — Refine's own example uses the same guard); (4) optionally listen for `PASSWORD_RECOVERY` in `onAuthStateChange` before showing the form; (5) add an e2e test (local Supabase → Inbucket email → click link → set new password → log in) covering the confirm → update-password hop. Known tradeoff: the `/auth/v1/verify` link is consumed on a plain GET, so an email scanner that pre-fetches links can burn it ("link expired"); the rejected option (a) — an SPA `/auth/confirm` route calling `verifyOtp` plus a `vercel.json` SPA rewrite and a relative-only `next` check — avoids that but is more code and not the documented SPA path. **Update 2026-09-21:** #283 fixed the templates/route guard, but a hard load of `/update-password` on staging still returned Vercel's `404 NOT_FOUND` — `apps/web-next/vercel.json` had no SPA rewrite, and Vercel's Vite docs say deep linking "won't work out of the box" for SPAs. Fixed by adding `"rewrites": [{ "source": "/(.*)", "destination": "/index.html" }]` (this also fixes hard refresh of any non-root route). **Confirmed fixed on staging 2026-09-23**: `/update-password` and `/transactions` both load correctly (no more Vercel 404), and a real password-reset email end-to-end (request → email → correct Supabase link → set new password → log in with it) succeeded. **Still open: prod.** Needs the same two steps done there before this can be considered fully resolved: (1) add `https://moneylens-mocha.vercel.app/update-password` to the Redirect URLs allow-list in the prod Supabase dashboard, (2) paste the updated `recovery.html`/`magic-link.html` templates into the prod dashboard (hosted projects don't read `supabase/templates/`) — both already shipped to `main` via #283/#284, prod just needs the dashboard-side and Vercel deploy to catch up at the next release.
- [x] ~~Transactions list can hide a row depending on page size~~ — reported in production 2026-08-24 (council tax transaction on 11/08/2026 missing at 10-items-per-page, visible at 20/50); fixed in #274. **Confirmed and fixed**: reproduced directly against the `transactions_with_details` REST endpoint by seeding 22 same-date rows and comparing paginated fetches at different page sizes — paging at size 10 returned only 23 unique ids across 30 fetched rows (6 duplicates, meaning other rows were silently skipped). Root cause: `transactions/list.tsx`'s sorter had no tie-breaker column, so without a fully-deterministic `ORDER BY`, successive `LIMIT`/`OFFSET` pages over same-`date` rows aren't guaranteed consistent. Fixed by always appending `created_at, id` as final sort keys — `created_at` keeps same-date rows newest-first, `id` (the PK) guarantees a total order. Injected in the initial sorter and synchronously in the Table's `onChange` handler (so no non-deterministic query is ever issued), plus a mount-only `useEffect` that normalizes sorters restored from a pre-fix URL. A cleared sort falls back to the default `date desc` view rather than a bare tie-breaker order. Covered by a new e2e regression test (`transactions.spec.ts`, "every transaction on a tied sort date appears exactly once when paging at page size 10") that asserts the outgoing request carries `order=date.desc,created_at.desc,id.desc`, that all tied rows appear exactly once across pages at page size 10, and that the primary `date` sort still dominates the tie-breakers.
...
- [ ] Component test infrastructure — unit-test infra now exists (`vitest`, a `test:unit` script, and `src/utility/*.test.ts` files), but `@testing-library/react` and a jsdom/happy-dom environment are still missing, which blocks all component-render tests (transaction form validation, budget progress display, transaction filters), not just the individual missing ones below
...
- [ ] Small unit-test gaps: `formatAmount`/`formatCurrency` (`utility/currency.ts`), `getMonthKeysInRange` (`utility/monthHelpers.ts`) — both still untested. (`slugify` was previously listed here but no such helper exists in `src/`; the only `slugify` is a local test helper in `e2e/utils/test-helpers.ts`.)
