...
- [ ] **Transactions list can hide a row depending on page size (reported in production, 2026-08-24)** — user entered a transaction (council tax, 11/08/2026, £85, Barclays) that didn't appear in the list at 10-items-per-page even after paging to the date, but was visible at 20 or 50 items per page. Suspected root cause: `transactions/list.tsx:174-176`'s `useTable` sorter is `[{ field: "date", order: "desc" }]` with no tie-breaker column. Without a secondary deterministic sort key (e.g. `id` or `created_at`), Postgres doesn't guarantee stable ordering among rows sharing the same `date` across separate paginated `.range()` queries — the query plan (and thus tie order) can differ between `LIMIT`/`OFFSET` values, so a tied row can fall through a page boundary and appear to vanish at one page size but not another. Needs confirming against a seeded multi-transaction-same-date fixture, then likely fixed by adding a secondary sort key (e.g. `id`) to the initial sorter so ordering is fully deterministic.
